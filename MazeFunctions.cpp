...
// INITIALIZE MAZE
void MazeFunctions::Initialize(Cell Level[][SIZE]) {
...
// CLEAR SCREEN
void MazeFunctions::ClearScreen()
{
	HANDLE hOut;
	COORD Position;
...
// REDRAW MAZE
void MazeFunctions::Redraw(Cell Level[][SIZE]) {
	for (int i = 0; i < SIZE; i++) {
		cout << endl;
		for (int j = 0; j < SIZE; j++)
...
// GENERATE MAZE
void MazeFunctions::GenerateMaze(Cell Level[][SIZE], int &posX, int &posY, int &goalX, int &goalY) {
...
// SAVE MAZE
void MazeFunctions::SaveMaze(Cell Level[][SIZE]) {
	ofstream output;
	char file[20];
	char input;
